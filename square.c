...
int main() {
    ...
    for (int i = 0; i < num_samples; i++) {
        double phase = fmod(frequency * i / sample_rate, 1.0);
        double sample = amplitude * (phase < 0.5 ? 1.0 : -1.0);
    ...
    fclose(fp);
    printf("Created square.wav: %d Hz, %.1f seconds\n", (int)frequency, duration);
    return 0;
}
