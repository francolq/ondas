...
int main() {
    ...
    FILE *fp = fopen("sine_stereo.wav", "wb");
    if (!fp) {
    ...
    for (int i = 0; i < num_samples; i++) {
        ...
        double pan = (double)i / num_samples;
        double left = amplitude * sine * (1.0 - pan);
        double right = amplitude * sine * pan;
        short l = (short)left;
        short r = (short)right;
    ...
    fclose(fp);
    printf("Created sine_stereo.wav: %d Hz, %.1f seconds, stereo panning left to right\n", (int)frequency, duration);
    return 0;
}
