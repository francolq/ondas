...
class DrumMachine(App):
    ...
    def toggle_step(self, step_num):
        self.steps[self.selected_pad][step_num] = not self.steps[self.selected_pad][step_num]
        self.update_step_display()
    ...
    def audio_callback(self, outdata, frames, time, status):
        ...
        while self.playing and self.step_samples_accumulated >= SAMPLES_PER_STEP:
            ...
            triggered = True
            for pad in range(16):
                if self.steps[pad][self.current_step]:
                    sample = self.samples[pad]
                    if sample is not None:
                        with self.audio_lock:
                            self.current_voice = Voice(sample.copy(), 1.0)
        if triggered:
            self.call_from_thread(self.update_step_display)

    def update_step_display(self):
        for i in range(16):
            button = self.query_one(f"#step_{i}", Button)
            is_on = self.steps[self.selected_pad][i]
            is_current = (i == self.current_step) and self.playing
            button.label = "●" if is_on else "○"
            button.remove_class("step-off", "step-on", "current-step")
            if is_on:
